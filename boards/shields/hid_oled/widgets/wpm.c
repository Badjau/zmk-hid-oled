...
#if IS_ENABLED(CONFIG_HID_OLED_GEM_WPM_FIXED_RANGE)
    float max = CONFIG_HID_OLED_GEM_WPM_FIXED_RANGE_MAX;
#else
    float max = 0;
    for (int i = 0; i < 10; i++) {
        if (state->wpm[i] > max) {
...
#if IS_ENABLED(CONFIG_HID_OLED_GEM_WPM_FIXED_RANGE)
    int max = CONFIG_HID_OLED_GEM_WPM_FIXED_RANGE_MAX;
    if (max == 0) {
        max = 100;
    }
...
static void draw_label(lv_obj_t *canvas, const struct status_state *state) {
    ...
    init_label_dsc(&label_left_dsc, LVGL_FOREGROUND, LV_FONT_DEFAULT, LV_TEXT_ALIGN_LEFT);
    lv_canvas_draw_text(canvas, 0, 101 + BUFFER_OFFSET_MIDDLE, 25, &label_left_dsc, "WPM");

    lv_draw_label_dsc_t label_dsc_wpm;
    init_label_dsc(&label_dsc_wpm, LVGL_FOREGROUND, LV_FONT_DEFAULT, LV_TEXT_ALIGN_RIGHT);
